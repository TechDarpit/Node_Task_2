...
const UserData: React.FC<{ usersData: DisplayUser[]; searchValue: string }> = (
  props
) => {
  const { usersData, searchValue } = props;
  // console.log(usersData[0].tasksCount);
  ...
  const [usersList, setUsersList] = useState<DisplayUser[]>(usersData);

  // console.log(usersList[0].lastName);
  ...
  const sortBy = <User, K extends keyof User>(
    ...
    const sortedusers = users.sort((a: User, b: User) => {
      ...
      } else if (dateFieldName === 'tasksCount') {
        if (sortDirection[dateFieldName]) {
          return a[dateFieldName] - b[dateFieldName];
        } else {
          return b[dateFieldName] - a[dateFieldName];
        }
      }
          ...
          <Table hover bordered variant='light'>
            <thead>
              <tr>
                ...
                <th onClick={() => sortBy(usersList, 'tasksCount')}>
                  Task Count
                </th>
                {/* <th>Total Tasks</th> */}
                <th>Actions</th>
