...
exports.getUsers = async (req, res, next) => {
  try {
    const users = await sequelize.query(
      "SELECT `users`.`uId`, `users`.*, COUNT(`tasks`.`userUId`) AS 'tasksCount' FROM `users` AS `users` LEFT OUTER JOIN `tasks` AS `tasks` ON `users`.`uId` = `tasks`.`userUId` GROUP BY `users`.`uId`;"
    );
    if (!users) {
